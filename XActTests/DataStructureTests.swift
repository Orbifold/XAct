...
class GraphTests: XCTestCase {
    ...
    func testCreate() {
        var g = ObjectGraph()
        var one = ObjectNode(id: 1);
        var two = ObjectNode(id: 2);
        ...
        XCTAssertEqual(one.AllEdges.count, 1)
        XCTAssertEqual(two.AllEdges.count, 1)
        XCTAssertEqual(one.Outgoing[0], edge)
        XCTAssertEqual(two.Incoming[0], edge)
        g.RemoveEdge(edge)
        XCTAssertEqual(one.AllEdges.count, 0)
        XCTAssertEqual(two.AllEdges.count, 0)
    ...
    func testNeighbors(){
        ...
        XCTAssertEqual(one.Neighbors.count, 2)
        XCTAssertTrue(contains(one.Neighbors, two))
           XCTAssertTrue(contains(one.Neighbors, three))
    }
